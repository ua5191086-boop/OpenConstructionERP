...
CREATE INDEX IF NOT EXISTS idx_bim_project ON bim_models(project_id);
CREATE INDEX IF NOT EXISTS idx_bim_discipline ON bim_models(discipline);
CREATE INDEX IF NOT EXISTS idx_bim_latest ON bim_models(project_id, is_latest) WHERE is_latest = TRUE;
...
CREATE TABLE bim_elements (
    ...
    boq_item_id     UUID REFERENCES boq_items(id),       -- привязка к смете
    created_at      TIMESTAMPTZ NOT NULL DEFAULT NOW()
);

CREATE INDEX IF NOT EXISTS idx_bim_elements_model ON bim_elements(model_id);
CREATE INDEX IF NOT EXISTS idx_bim_elements_type ON bim_elements(ifc_type);
CREATE INDEX IF NOT EXISTS idx_bim_elements_level ON bim_elements(level);
...
CREATE TABLE bim_relationships (
    id              UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    model_id        UUID NOT NULL REFERENCES bim_models(id) ON DELETE CASCADE,
    source_element_id UUID NOT NULL REFERENCES bim_elements(id) ON DELETE CASCADE,
    target_element_id UUID NOT NULL REFERENCES bim_elements(id) ON DELETE CASCADE,
    relationship_type VARCHAR(100) NOT NULL,                -- IfcRelContainedInSpatialStructure, IfcRelVoidsElement, IfcRelFillsElement
    description     TEXT,
    created_at      TIMESTAMPTZ NOT NULL DEFAULT NOW()
);

CREATE INDEX IF NOT EXISTS idx_bim_rel_source ON bim_relationships(source_element_id);
CREATE INDEX IF NOT EXISTS idx_bim_rel_target ON bim_relationships(target_element_id);
...
CREATE INDEX IF NOT EXISTS idx_bim_clashes_model ON bim_clashes(model_id);
CREATE INDEX IF NOT EXISTS idx_bim_clashes_status ON bim_clashes(status);
CREATE INDEX IF NOT EXISTS idx_bim_clashes_severity ON bim_clashes(severity);
...
CREATE TABLE bim_markups (
    ...
    assigned_to     UUID REFERENCES employees(id),
    parent_id       UUID REFERENCES bim_markups(id),      -- для тредов
    created_at      TIMESTAMPTZ NOT NULL DEFAULT NOW()
);

CREATE INDEX IF NOT EXISTS idx_bim_markups_model ON bim_markups(model_id);
CREATE INDEX IF NOT EXISTS idx_bim_markups_element ON bim_markups(element_id);
...
CREATE INDEX IF NOT EXISTS idx_bim_4d_element ON bim_4d_links(element_id);
